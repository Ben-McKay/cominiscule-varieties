...
Our eyes immediately spot in that Hasse diagram its uppermost component, which is always the Hasse diagram of a unique cominuscule subvariety.
...
We will see that all free smooth subvarieties are homogeneous, providing evidence for the conjecture that the associated cominuscule subvariety is the unique free smooth subvariety.
\begin{example}
...
\end{tikzpicture}
\]
Hence the associated cominuscule subvariety is invariant under a Cartan subgroup, and conversely there are three associated cominuscule subvarieties invariant under any given Cartan subgroup.
...
\]
from which we cut out the crosses and their edges, leaving only the affine node \(\dynkin A{o}\), and then turn the affine node \(\dynkin A{o}\) into a cross \(\dynkin A{x}\), giving us \(\dynkin A{x}\), the Dynkin diagram of the projective line, our rational curve.
\end{example}
...
\end{lemma}
\begin{proof}
Since \(G\) acts transitively on \(X\), it suffices to prove the conjugacy of any two associated cominuscule flag varieties through the same point \(x_0\in X\), hence stabilized by the same parabolic subgroup \(P:=G^{x_0}\).
...
\end{proof}
The tangent space to \(\breve X\) at \(x_0\) is \(\op\LieZ\); intuitively the associated cominuscule variety arises by integrating \(G\)-translates of that subspace.
...
Each flag variety is determined uniquely, up to finite central extension of \(G\) and up to isomorphism, by its \emph{Dynkin diagram}, which is the Dynkin diagram of \(G\) decorated with \(\dynkin{A}{*}\) on each compact simple root and \(\dynkin{A}{x}\) on each noncompact simple root \cite{Borel:1991} p.~197 Proposition 14.18, \cite{Humphreys:1975} p.~197 Theorem${}'$.
\subsection{Affine extensions}
Every Dynkin diagram sits inside its associated extended Dynkin diagram \cite{Bourbaki:2002} p.~198, \cite{Kac:2010} lecture~17, \cite{OnishchikVinberg:1990} p.~164, \(5^{\text{o}}\), given by adding one node to each component of the original Dynkin diagram, which we call the \emph{affine node}, drawn as a hollow dot \(\dynkin A{o}\), with edges attached as:
\begingroup
\small
\begin{longtabl}{@{}%
>{\columncolor[gray]{.9}$}r<{$}%
...
\end{longtabl}
\endgroup
%Parabolic geometers will note that this table extends table~3.2 of \cite{Baston/Eastwood:1989} p.~27.
...
Pick a root vector \(e_{\aa}\in\LieZ\) for a root \(\aa\).
...
That subalgebra is generated in the same way, by the box root vectors, as every complex semisimple Lie group is generated from the root vectors of a basis of roots \cite{Serre:2001} p.~43 Theorem~1, so \(\breve\LieG\) is that subalgebra, so is complex semisimple.
...
Suppos that every such root string has a single root in it.
...
Hence the \(P\)-compact roots divide into (1) \(\breve\Roots\), i.e. those roots which are differences of \(P\)-maximal roots, i.e. \(\breve{P}\)-compact roots and (2) those perpendicular to \(\breve\Roots\), forming a root subsystem of the \(P\)-compact roots giving a complex semisimple subgroup of \(\GZ\).
...
Recalle that \(\LieZ\) is an irreducible \(\GZ\)-module.
...
As we do this, we repeatedly passing between \(P\)-maximal roots \(\aa,\bb\) by subtracting a \(\breve{P}\)-compact positive root \(\aa-\bb\).
...
If \(\aa\) is \(P\)-compact, its root space lies in \(\LieGZ\).
So we can assume that \(\aa\) is a \(P\)-noncompact root.
...
(Changing sign of each root takes positive roots to negative roots and is an isometry, arising from our automorphism \(a\) as above, mirror reflecting the Hasse diagram, so when we draw the Hasse diagram, the reader can think of it as drawing positive or drawing negative roots, as required.)
\begin{example} 
...
By knocking out the relevant edges, we see that the boxes of either of these flag varieties consist of a single \(\dynkin A{x}\).
...
The resulting Dynkin diagram has precisely one component with a crossed node, the Dynkin diagram of the associated cominuscule subvariety, consisting precisely of the new crossed node and the uncrossed nodes whose labels arise along the edges of the box.
\end{lemma}
\begin{proof}
...
By lemma~\vref{lemma:box.roots}, every \(P\)-compact root is a \(P'\)-compact root and vice versa. 
The \(\breve P\)-compact roots are precisely those \(P\)-compact roots which are differences of box roots, i.e.\ which appear as labels in the edges of the box.
...
By lemma~\vref{lemma:associated.cominuscule}, \((\breve X,\breve G)\) is irreducible cominuscule.
...
In the \((\breve X,\breve G)\)-Dynkin diagram, by the standard rules of construction of Dynkin diagrams, the new crossed node (corresponding to \(\breve\alpha\)) is connected to another node \(\alpha_j\) (hence to an uncrossed node) precisely when their root vectors have nonzero bracket, so precisely when the root \(\breve\alpha\) has an edge labelled by \(j\).
\end{proof}
\begin{example}
...
We can see that the associated cominuscule subvariety is the Grassmannian, given by throwing away roots \(4,5,6\) and the components of the Hasse diagram which contain them.
\end{example}

To compute the Dynkin diagram of the associated cominuscule subvariety \((\breve X,\breve G)\) of a flag variety \((X,G)\), we need to knock out edges from the Hasse diagram of \(G\).
...
Root \(2\) is the unique root going up from the lowest box root, while root \(4\) is the unique root going down from the highest root of the box:
\[
...
\begin{example}
Imagine computing the associated cominuscule subvariety of an irreducible flag variety.
...
In this section, we prove that the associated cominuscule subvariety in any irreducible flag variety is preserved by a subgroup of the automorphisms of the flag variety which is as large as possible subject to an open condition on tangent spaces.
...
\]
clearly an antisymmetric bilinear map.
